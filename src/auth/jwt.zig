...
pub const JWT = struct {
    ...
    pub fn create(self: JWT, claims: Claims) ![]const u8 {
        ...
        defer payload_json.deinit(self.allocator);
        
        // Manually construct JSON (simpler than finding the right std.json API)
        const writer = payload_json.writer(self.allocator);
        try writer.print("{{\"user_id\":{},\"email\":\"{s}\",\"username\":", .{ claims.user_id, claims.email });
        if (claims.username) |un| {
            try writer.print("\"{s}\"", .{un});
        } else {
            try writer.writeAll("null");
        }
        try writer.print(",\"exp\":{},\"iat\":{}}}", .{ claims.exp, claims.iat });
        
        const payload_b64 = try self.base64urlEncode(payload_json.items);
