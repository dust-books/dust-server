...
pub const PermissionRepository = struct {
    ...
    pub fn getUserPermissions(self: *PermissionRepository, user_id: i64) !std.ArrayList(Permission) {
        var perms = std.ArrayList(Permission).init(self.allocator);
        errdefer perms.deinit();
        ...
        while (try stmt.step()) {
            const perm = Permission{
                ...
                .description = if (stmt.columnIsNull(2)) null else try self.allocator.dupe(u8, stmt.columnText(2)),
                .resource = try self.allocator.dupe(u8, stmt.columnText(3)),
                .action = try self.allocator.dupe(u8, stmt.columnText(4)),
                .created_at = try self.allocator.dupe(u8, stmt.columnText(5)),
            };
    ...
    pub fn getUserRoles(self: *PermissionRepository, user_id: i64) !std.ArrayList(Role) {
        ...
        while (try stmt.step()) {
            const role = Role{
                ...
                .name = try self.allocator.dupe(u8, stmt.columnText(1)),
                .description = if (stmt.columnIsNull(2)) null else try self.allocator.dupe(u8, stmt.columnText(2)),
                .created_at = try self.allocator.dupe(u8, stmt.columnText(3)),
            };
